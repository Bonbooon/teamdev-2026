...
get_demo_user_email() {
  ...
  local email
  email=$(grep -E '^DEMO_USER_EMAIL=' "$APP_ENV_FILE" | tail -n 1 | cut -d '=' -f 2- | tr -d '\r' || true)
  email="${email%\"}"
  email="${email#\"}"
  printf '%s' "$email"
}
...
ensure_interactive_terminal() {
  ...
  echo "Error: mise run laravel-init requires interactive input for the demo user Google login step."
  echo "Run it from an interactive terminal after setting DEMO_USER_EMAIL in $APP_ENV_FILE."
  exit 1
}

verify_demo_user_login() {
...
if (!is_string($email) || $email === '') {
    fwrite(STDERR, "DEMO_USER_EMAIL is not configured.\n");
    exit(2);
}
...
prompt_for_demo_user_login() {
  local demo_email
  local prompt_label

  while true; do
    demo_email="$(get_demo_user_email)"

    echo ""
    echo "Database reset completed. Demo data seeding requires a Google-authenticated seed user."
    if [ -n "$demo_email" ]; then
      echo "DEMO_USER_EMAIL is currently: $demo_email"
    else
      echo "DEMO_USER_EMAIL is not set in $APP_ENV_FILE"
    fi
    echo ""
    echo "Before seeding:"
    echo "  1. Update $APP_ENV_FILE so DEMO_USER_EMAIL matches the Google account you will use"
    ...
    if verify_demo_user_login; then
      demo_email="$(get_demo_user_email)"
      echo "Seed user detected for ${demo_email:-DEMO_USER_EMAIL}. Continuing with seeding..."
      return 0
    fi

    demo_email="$(get_demo_user_email)"
    echo "Seed user was not found for ${demo_email:-<unset>} yet."
    echo "Update DEMO_USER_EMAIL if needed, finish the Google login flow, then try again."
  done
}
