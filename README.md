...
## 概要
[プログラミング学習コミュニティPOSSE](https://posse.community)主催の２ヶ月間にわたるハッカソン式のソフトウェア開発大会で８チーム中優勝を目指して競いました。
企業様からお題をいただき、プロトタイプの企画・施策を行い、最後にプレゼンテーションをしました。
3人チームで私はチームリーダーとして要件定義・開発・テスト・プレゼンテーションまで一気通貫で関わり、３位入賞を成し遂げました。

## 制作背景

### 課題

既存のタスク管理ツールは、課題の登録や担当管理には強い一方で、プロジェクトが遅れそうな兆候を早期に捉えたり、遅延時の次アクションまで支援したりする部分は弱い背景がありました。特にチーム開発では、進捗の遅れとメンバーの負荷・状態が連動して悪化しやすく、単純なタスク一覧だけでは状況判断が難しくなります。
...
- バックエンドはクリーンアーキテクチャ（ `Domain / Application / Infrastructure / Interfaces` ）を意識した構成です
