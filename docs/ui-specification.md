...
| パス | ページ名 | 要件ID | 認証 | ロール | 説明 |
|------|----------|--------|------|--------|------|
| `/login` | ログイン | S-01-01, S-01-03 | 不要 | 全員 | Google OAuthログイン |
| `/profile/setup` | プロフィール登録 | S-01-02 | 必要 | 全員 | 初回登録・編集 |
| `/` | ダッシュボード | S-08-01, S-08-02 | 必要 | 全員 | ロール別トップページ |
| `/teams` | チーム一覧 | S-04-01, S-04-02 | 必要 | 全員 | マネージャー: 管轄チーム / メンバー: 所属チーム |
| `/teams/[teamId]` | チーム詳細 | S-04-03 | 必要 | 全員 | タブ: プロジェクト一覧, メンバー一覧 |
| `/projects` | プロジェクト一覧 | — | 必要 | 全員 | 参加中プロジェクト一覧 |
| `/projects/[projectId]` | プロジェクト詳細 | S-05-04 | 必要 | 全員 | 進捗ボード, ガントチャート, アラート |
| `/projects/[projectId]/issues/new` | Issue作成 | S-03-01 | 必要 | 全員 | SMARTテンプレート付きIssue作成 |
| `/issues/[issueId]` | Issue詳細 | S-03-05, S-03-06 | 必要 | 全員 | サブタスク, 進捗, ステータス管理 |
| `/alerts` | アラート一覧 | S-02-01, S-02-02 | 必要 | 全員 | 横断的アラート一覧 |
| `/surveys` | サーベイ | S-05-01 | 必要 | 全員 | パルスサーベイ回答 |
| `/users/[userId]` | プロフィール閲覧 | S-06-02 | 必要 | 全員 | 他メンバーのプロフィール表示 |
